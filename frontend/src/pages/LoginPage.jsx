  ...
  return (
    <div className="min-h-[85vh] bg-[#faf7f2] dark:bg-[#141210] flex items-center justify-center px-4 py-12 sm:px-6 lg:py-16 relative overflow-hidden text-[#2d241e] dark:text-[#ede4d8] transition-colors duration-300">
      <div className="w-full max-w-md relative z-10">
        {/* Main Card */}
        <div className="bg-white dark:bg-[#1e1915] rounded-3xl p-7 sm:p-10 shadow-[0_15px_45px_-15px_rgba(59,45,36,0.08)] border border-[#ebdcca] dark:border-[#2d241c]">
          {/* Header & Logo */}
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label
                  htmlFor="password"
                  className="block text-sm font-semibold uppercase tracking-wider text-[#3b2d24] dark:text-[#ede4d8]"
                >
                  {language === "ru" ? "Пароль" : "Parol"}
                </label>
                <Link
                  to="/forgot-password"
                  className="text-xs sm:text-sm font-semibold text-[#8a735e] dark:text-[#c4a98e] hover:text-[#3b2d24] dark:hover:text-[#ede4d8] transition-colors"
                >
            ...
            {/* Error Message */}
            {error && (
              <div className="flex items-start gap-3 p-4 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm leading-relaxed animate-fadeIn">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full group relative flex items-center justify-center gap-2 py-4 px-6 rounded-xl bg-[#3b2d24] dark:bg-[#c1a27c] text-white dark:text-[#1e1915] font-semibold text-base transition-all duration-300 shadow-md hover:bg-[#271f19] dark:hover:bg-[#d6ba94] hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {submitting ? (
          ...
          {/* Footer Navigation */}
          <div className="mt-8 pt-6 border-t border-[#ebdcca] dark:border-[#2d241c] text-center">
            <p className="text-sm text-[#6b584a] dark:text-[#b8a99a]">
              {language === "ru" ? "У вас еще нет аккаунта?" : "Hali ro‘yxatdan o‘tmadingizmi?"}{" "}
              <Link
                to="/register"
                className="font-bold text-[#3b2d24] dark:text-[#c1a27c] hover:underline ml-1"
              >
