...
function ContactPage() {
  ...
  const contactChannels = [
    {
      icon: Phone,
      title: language === "ru" ? "Телефон для заказов" : "Buyurtmalar uchun telefon",
  ...
  return (
    <div className="bg-[#faf7f2] min-h-screen py-10 sm:py-16 text-[#2d241e]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl">
          ...
          <h1 className="mt-2 font-serif text-3xl sm:text-5xl font-bold text-[#3b2d24] tracking-tight">
            {language === "ru" ? "Контакты и служба заботы" : "Aloqa va mijozlarga xizmat"}
          </h1>
          <p className="mt-3 text-base sm:text-lg text-[#6b584a] leading-relaxed">
            {language === "ru"
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {contactChannels.map((ch, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white rounded-2xl p-6 border border-[#ebdcca] shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="w-12 h-12 rounded-xl bg-[#faf7f2] text-[#8a735e] border border-[#dfd2c0]/60 flex items-center justify-center mb-4">
                  <Icon className="w-6 h-6" />
                </div>
                <p className="text-xs sm:text-sm font-medium text-[#8a735e] uppercase tracking-wider">
                  {ch.title}
                </p>
                {ch.link ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-1.5 block font-bold text-[#3b2d24] text-base sm:text-lg hover:text-[#8a735e] transition-colors"
                  >
                    {ch.value}
                  </a>
                ) : (
                  <p className="mt-1.5 font-bold text-[#3b2d24] text-base sm:text-lg">
                    {ch.value}
                  </p>
                )}
                <p className="mt-1.5 text-xs sm:text-sm text-[#7a6758]">{ch.desc}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="mt-12 grid lg:grid-cols-12 gap-8 items-start">
          {/* Form */}
          <div className="lg:col-span-7 bg-white rounded-3xl p-7 sm:p-10 border border-[#ebdcca] shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={handleChange}
                      autoComplete="tel"
                      placeholder="+998 93 079 17 34"
                      className="w-full pl-12 pr-4 py-3.5 text-base bg-[#faf7f2]/60 border border-[#d6c6b3] rounded-xl outline-none transition-all placeholder:text-stone-400 focus:bg-white focus:border-[#3b2d24] focus:ring-2 focus:ring-[#8a735e]/15"
                    />
                    <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#8a735e]" />
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  <textarea
                    ...
                    className="w-full p-4 text-base bg-[#faf7f2]/60 border border-[#d6c6b3] rounded-xl outline-none transition-all placeholder:text-stone-400 resize-none focus:bg-white focus:border-[#3b2d24] focus:ring-2 focus:ring-[#8a735e]/15"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-3 py-4 px-9 rounded-full bg-[#3b2d24] text-white font-semibold text-base transition-all duration-300 shadow-md hover:bg-[#271f19] hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitting ? (
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="rounded-3xl bg-[#3b2d24] text-white p-8 shadow-xl relative overflow-hidden">
              <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center text-[#c1a27c] mb-5">
              ...
              <div className="mt-6 pt-5 border-t border-white/10 space-y-3 text-sm text-stone-200">
                <div className="flex items-center justify-between">
                  <span className="text-stone-300">{language === "ru" ? "Стоимость:" : "Yetkazish:"}</span>
                  <span className="font-bold text-white">0 UZS (Bepul)</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-stone-300">{language === "ru" ? "Оплата:" : "To‘lov:"}</span>
                  <span className="font-bold text-[#c1a27c]">{language === "ru" ? "Наличными курьеру" : "Qabul qilganda naqd"}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-stone-300">{language === "ru" ? "Сроки:" : "Yetkazish muddati:"}</span>
            ...
            {/* Quick FAQ Card */}
            <div className="bg-white rounded-3xl p-7 sm:p-8 border border-[#ebdcca] shadow-sm">
              <div className="flex items-center gap-2.5 mb-5">
                <HelpCircle className="w-6 h-6 text-[#8a735e]" />
              ...
              <div className="space-y-4 divide-y divide-stone-100">
                {faqs.map((faq, idx) => (
                  <div key={idx} className={idx > 0 ? "pt-4" : ""}>
                    <p className="text-sm sm:text-base font-bold text-[#3b2d24]">{faq.q}</p>
                    <p className="mt-1.5 text-sm text-[#6b584a] leading-relaxed">{faq.a}</p>
                  </div>
                ))}
              </div>
            </div>
